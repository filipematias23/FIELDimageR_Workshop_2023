...
EX.Table.Parallel <- foreach(i = 1:length(pics), .packages = c("raster","FIELDimageR","terra","parallel"), 
                             .combine = rbind) %dopar% {
...
EX3.Data<-fieldInfo_extra(mosaic = EX3.I[[c("SI","BGI","BI","VARI")]], 
                          fieldShape = as(EX3.D$Objects,"SpatVector"))

# Combining the data:
Data.Obj<-cbind.data.frame(EX3.Data,EX3.D$Dimension)
Data.Obj

# Data visualization: 
Data.Obj1<-melt(Data.Obj) #,id.vars = c("SI","BGI","BI","VARI","area","x.dist","y.dist")
